...
class MemberGNN(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        # self.gnn = GCN(
        #     in_channels = hparams["emb_hidden"],
        #     hidden_channels = hparams["emb_hidden"],
        #     num_layers = hparams["gnn_layers"],
        #     out_channels = hparams["emb_hidden"],
        #     # v2=True,
        #     norm="LayerNorm"
        # )
        ...
        output_size = 2*hparams["emb_hidden"]
    ...
    def message_passing(self, x, edge_index):
        ...
        scattered_node_features = scatter_add(x[start], end, dim=0, dim_size=x.shape[0])
    ...
    def forward(self, x, edge_index):
        ...
        for _ in range(self.hparams["gnn_layers"]):
        ...
        return self.output_classifier(edge_features)
    ...
    def training_step(self, batch, batch_idx):
        ...
        classification_score = self(batch.x, batch.edge_index)
        # print(classification_score.mean(), classification_score.std(), (classification_score > 0).sum())

        # Apply a loss function with a positive weight
        loss = F.binary_cross_entropy_with_logits(
            classification_score.squeeze(), batch.y.squeeze(), pos_weight=torch.tensor([self.hparams["weight"]]).to(self.device)
        )
    ...
    def shared_evaluation(self, batch):
        ...
        # Apply the model to the input data
        classification_score = torch.sigmoid(self(batch.x, batch.edge_index))
